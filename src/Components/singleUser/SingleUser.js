...
<React.Fragment>
  {isLoading?<Spinner />:(<div className={classes.main}>
     ...
     <div className={classes.text}>
      <h1 style={{textAlign:'center',marginLeft:'100px',color:'black',fontSize:'16px'}} className='ui button'>Dear, <h5 style={{display:'inline-block',color:'blue',fontWeight:'bold',marginRight:'3px'}}>{user.username}</h5> welcome to your profile</h1>
    <div className={classes.icons}>
 {user.places.length !==0 && <Link to={`/myblog/${user.id}`}><button className='ui negative button'>You shared {user.places.length}  posts </button></Link>}
